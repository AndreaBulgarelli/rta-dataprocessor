...
if [ -f "$base_path/keys/id_rsa" ] && [ -f "$base_path/keys/id_rsa.pub" ]; then
    ...
    chmod -R 600 $HOME/.ssh/id_rsa
    chmod -R 644 $HOME/.ssh/id_rsa.pub

    cp $base_path/keys/authorized_keys $HOME/.ssh/authorized_keys
    chown -R astrisw:astrima $HOME/.ssh/authorized_keys
